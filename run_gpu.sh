...
: "${BASE_MODEL:?set BASE_MODEL=<hf repo id of the starting checkpoint>}"
: "${BENCH_REPO:?set BENCH_REPO=<hf dataset repo id containing benchmark_1000.csv>}"
[ -d models/pii-guard-turkish-270m ] || python -c "import os; from huggingface_hub import snapshot_download as s; s(os.environ['BASE_MODEL'], local_dir='models/pii-guard-turkish-270m')"
[ -f data/benchmark_1000.csv ] || python -c "import os; from huggingface_hub import hf_hub_download as h; h(os.environ['BENCH_REPO'],'benchmark_1000.csv',repo_type='dataset',local_dir='data')"
